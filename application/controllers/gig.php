...
class Gig extends CI_Controller {
	...
	{	//base_url sets url for pagination links,
		//total_rows sets total number of items in table
		//per_page sets limit for number of items taken from db
		$config['base_url'] = base_url() . 'index.php/gig/index';
		$config['total_rows'] = $this->Gig_model->num_records();
		$config['per_page'] = 10;
		
		//num_links sets the maximum number of links allowed in pagination
		//uri segment sets position in base_url for starting position
		//full_tag open and close puts pagination links in div
		$config['num_links'] = 20;
		...
		//if there is a uri_segment(3) exists, then $page equals
		//that segment, otherwise, $page is 0
		$page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		
		//get gigs within the limit set by per_page,
		//use $page as starting point
		$data['query'] = $this->Gig_model->get_gigs($config['per_page'], $page);
		$data['title'] = 'Gig Central';
		$data['banner'] = 'Welcome to Gig Central';
		$data['copyright'] = '&copy; Nathaniel Cohn';
		//get paginated links and put into data array
